...
> 目标：为 `C:\Users\23747\GolandProjects\yibuapi`（New API 分支，Go/Gin）构建一套全新的
...
- 对接 yibuapi 现有 `/api` 接口，**不改后端**（若确需新增接口，单独提出）。
...
### 已确定
- **站点名**：`yibuapi`（作为回落默认值；运行时仍以 `/api/status` 的 `system_name` 为准）
- **Logo**：Yibu 渐变图标，已下载至 `public/logo.png`，见 §1.6
...
主打统一 API、统一计费、分钟级接入、专属吞吐与 SLA。与 yibuapi 定位一致，
故其信息架构值得参考。
...
配色**不采用**其蓝色体系 —— 改用 Yibu 自有品牌色，见 §1.6。
...
### Logo
来源：`https://yibuapi-1320165265.cos.ap-guangzhou.myqcloud.com/yibuapi/logo/yibuapi-icon-jianbian-2.png`
已下载至 `new-fr/public/logo.png` —— 1242×1242 PNG，RGBA 透明底。

图形：圆角方形，**水平线性渐变**（蓝紫→天蓝→青绿）铺满，
其上为白色手写体字标 **Yibu**（大写 Y + 连笔 ibu）。
